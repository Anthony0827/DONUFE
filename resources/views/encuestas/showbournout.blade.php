...
<div class="radioFaces-container flex justify-center items-center flex-nowrap overflow-x-auto w-full">
    @for ($i = 1; $i <= 10; $i++)
        <div class="boxRadioFaces flex flex-col items-center">
            ...
            <span class="mt-2 text-gray-700 font-semibold text-sm" 
                style="height: 1.5rem; display: flex; align-items: center; justify-content: center;">
                {{ $i == 1 || $i == 10 ? $i : '' }}
            </span>

            <!-- Botón de radio estilizado -->
            <label for="r_{{ $i }}" class="mt-2 flex items-center">
                <input type="radio" id="r_{{ $i }}" name="respuesta_{{ $pregunta->id }}" value="{{ $i }}"
                    class="appearance-none w-5 h-5 border border-gray-300 rounded-full checked:bg-kulture-green checked:ring-2 checked:ring-kulture-green focus:outline-none focus:ring-2 focus:ring-kulture-green cursor-pointer">
            </label>
        </div>
    @endfor
</div>
                ...
                <!-- Feedback adicional -->
                  <div id="feedback_{{ $pregunta->id }}" class="feedback-container mt-8">
            ...
            <div class="flex flex-col space-y-4 md:flex-row md:space-y-0 justify-between md:items-end h-full pb-1">
                @if ($preguntaNumero > 1)
                    <button class="w-full lg:w-fit px-7 py-2.5 rounded-lg bg-kulture-green text-white shadow-md hover:bg-green-100 hover:text-kulture-green" id="btnAnterior" onclick="loadContent('{{ route('encuesta.showbournout', [$encuesta->id, $preguntaNumero - 1]) }}')">
                        <i class="bi bi-arrow-left-short"></i> Anterior
                    </button>
                @endif

                @if ($preguntaNumero < $totalPreguntas)
                    <button class="w-full lg:w-fit px-7 py-2.5 rounded-lg bg-kulture-green text-white shadow-md hover:bg-green-100 hover:text-kulture-green" id="btnSiguiente" onclick="guardarValorYContinuar('{{ route('encuesta.showbournout', [$encuesta->id, $preguntaNumero + 1]) }}', {{ $pregunta->id }})">
    ...
    id="btnFinalizar" 
    onclick="finalizarBournout({{ $pregunta->id }}, {{ Auth::user()->idcandidato }})">
    Finalizar
</button>
                @endif
            </div>
