...
<div class="radioFaces-container flex justify-center items-center flex-nowrap overflow-x-auto w-full">
    ...
    @endfor
            ...
            <div class="flex flex-col space-y-4 md:flex-row md:space-y-0 justify-between md:items-end h-full pb-1">
                @if ($preguntaNumero > 1)
                    <button class="w-full lg:w-fit px-7 py-2.5 rounded-lg bg-kulture-green text-white shadow-md hover:bg-green-100 hover:text-kulture-green"id="btnAnterior" onclick="loadContent('{{ route('encuesta.showfactoressociales', [$encuesta->id, $preguntaNumero - 1]) }}')">
                        <i class="bi bi-arrow-left-short"></i> Anterior
                    </button>
                @endif

                @if ($preguntaNumero < $totalPreguntas)
                    <button class="w-full lg:w-fit px-7 py-2.5 rounded-lg bg-kulture-green text-white shadow-md hover:bg-green-100 hover:text-kulture-green" id="btnSiguiente" onclick="guardarValorYContinuar('{{ route('encuesta.showfactoressociales', [$encuesta->id, $preguntaNumero + 1]) }}', {{ $pregunta->id }})">
    ...
    id="btnFinalizar" 
    onclick="finalizarFactoressociales({{ $pregunta->id }}, {{ Auth::user()->idcandidato }})">
    Finalizar
</button>
                @endif
            </div>
